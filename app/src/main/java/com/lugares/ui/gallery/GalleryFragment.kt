...
class GalleryFragment : Fragment(), OnMapReadyCallback{
    ...
    override fun onMapReady(map: GoogleMap) {
        map.let{
            googleMap = it
            mapReady= true
            //se instruye al map para que se vena lugares segun coordenadas
    ...
    private fun updateMap(lugares: List<Lugar>) {
        if (mapReady){
            lugares.forEach{ lugar -> if (lugar.latitud?.isFinite() ==true && lugar.logitud?.isFinite() == true){
                val marca = LatLng(lugar.latitud,lugar.logitud)
